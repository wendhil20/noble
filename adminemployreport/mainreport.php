...
// AUTO ROLLOVER: Check for incomplete tasks from previous week
function autoRolloverTasks($conn, $user_id, $current_week_start) {
    $previous_week_end = date('Y-m-d', strtotime('saturday last week', strtotime($current_week_start)));
    $previous_week_start = date('Y-m-d', strtotime('monday last week', strtotime($current_week_start)));
    
    // Get incomplete tasks from previous week
    $rollover_query = "SELECT * FROM employee_tasks 
                      WHERE user_id = $user_id 
                      AND start_date >= '$previous_week_start' 
                      AND start_date <= '$previous_week_end'
                      AND status != 'completed'
                      AND progress_percentage < 100
                      AND is_rolled_over = 0";
    
    $rollover_tasks = mysqli_query($conn, $rollover_query);
    $rolled_count = 0;
    
    while ($task = mysqli_fetch_assoc($rollover_tasks)) {
        // Store original week if first rollover
        $original_week = $task['original_week_start'] ?: $task['start_date'];
        $new_rollover_count = $task['rollover_count'] + 1;
        
        // Update the task to move to current week
        $update_query = "UPDATE employee_tasks SET 
                        start_date = '$current_week_start',
                        is_rolled_over = 1,
                        original_week_start = '$original_week',
...
// Run auto rollover at the start of each week
$rolled_count = autoRolloverTasks($conn, $_SESSION['user_id'], $current_week_start);
if ($rolled_count > 0) {
    $_SESSION['flash_message'] = "$rolled_count task(s) rolled over from previous week!";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['update_task'])) {
        ...
        $current_task = mysqli_fetch_assoc(mysqli_query($conn, "SELECT status, completed_date, delay_start_date, is_rolled_over FROM employee_tasks WHERE id=$task_id"));
        
        // Reset rollover status if task is completed
        $is_rolled_over = $current_task['is_rolled_over'];
        if ($status === 'completed') {
            $is_rolled_over = 0;
        }
        
        $completed_date = $current_task['completed_date'] ?? null;
        if ($status === 'completed' && ($current_task['status'] ?? '') !== 'completed' && !$completed_date) {
        ...
        $delay_days = 0;
        $delay_start_date = $current_task['delay_start_date'] ?? null;
        ...
        $query = "UPDATE employee_tasks SET 
                  task_title='$title', 
...
$current_tasks = mysqli_query($conn, "SELECT * FROM employee_tasks 
                                       ...
                                       ORDER BY is_rolled_over DESC, start_date ASC");
...
$stats_query = mysqli_query($conn, "SELECT 
                                     ...
                                     SUM(CASE WHEN is_rolled_over = 1 THEN 1 ELSE 0 END) as rolled_over_count
                                     FROM employee_tasks 
                                     WHERE user_id={$_SESSION['user_id']} 
                                     AND start_date >= '$current_week_start' 
                                     AND start_date <= '$current_week_end'");
...
<body class="bg-gray-50 min-h-screen">
    ...
    <div class="bg-white shadow-sm border-b border-gray-200">
        <div class="max-w-7xl mx-auto px-6 py-4 flex justify-between items-center">
           ...
           <a href="admin_dashboard.php" target="_blank" class="text-sm px-4 py-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition-colors font-medium flex items-center gap-2">
    ...
    <div class="max-w-full mx-auto px-6 py-6">
        <?php if ($message): ?>
            <div class="mb-6 p-4 rounded-lg text-sm shadow-sm <?php 
                if ($messageType === 'success') {
                    echo 'bg-green-50 text-green-700 border border-green-200';
                } elseif ($messageType === 'info') {
                    echo 'bg-blue-50 text-blue-700 border border-blue-200';
                } else {
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            ...
            <div class="mt-4 flex gap-6 text-sm flex-wrap">
                ...
                <?php if (($stats['rolled_over_count'] ?? 0) > 0): ?>
                <div class="flex items-center gap-2">
                    <svg class="w-4 h-4 text-orange-600" fill="currentColor" viewBox="0 0 20 20">
                        <path fill-rule="evenodd" d="M4 2a1 1 0 011 1v2.101a7.002 7.002 0 0111.601 2.566 1 1 0 11-1.885.666A5.002 5.002 0 005.999 7H9a1 1 0 010 2H4a1 1 0 01-1-1V3a1 1 0 011-1zm.008 9.057a1 1 0 011.276.61A5.002 5.002 0 0014.001 13H11a1 1 0 110-2h5a1 1 0 011 1v5a1 1 0 11-2 0v-2.101a7.002 7.002 0 01-11.601-2.566 1 1 0 01.61-1.276z" clip-rule="evenodd"></path>
                    </svg>
                    <span class="text-orange-600">Rolled Over:</span>
                    <span class="font-semibold text-orange-600"><?php echo $stats['rolled_over_count']; ?></span>
                </div>
                <?php endif; ?>
            </div>
        </div>
                            ...
                            <table class="w-full">
                                ...
                                <tbody>
                                    <?php while ($task = mysqli_fetch_assoc($current_tasks)):
                                        ...
                                        $is_rolled_over = $task['is_rolled_over'] == 1;
                                        $row_class = $is_rolled_over ? 'bg-orange-50 border-l-4 border-orange-400' : '';
                                    ?>
                                        <tr class="border-b border-gray-100 hover:bg-gray-50 transition-colors <?php echo $row_class; ?>">
                                            <td class="py-4 px-4">
                                                <div class="flex items-center gap-2">
                                                    <?php if ($is_rolled_over): ?>
                                                    <span class="flex-shrink-0 bg-orange-100 text-orange-700 text-xs px-2 py-1 rounded-full font-bold flex items-center gap-1">
                                                ...
                                                <?php if ($is_rolled_over && $task['original_week_start']): ?>
                                                <p class="text-xs text-orange-600 mt-1">
                                                    Originally from: <?php echo date('M d, Y', strtotime($task['original_week_start'])); ?>
                                                    <?php if ($task['rollover_count'] > 1): ?>
                                                    <span class="font-bold">(Rolled <?php echo $task['rollover_count']; ?>x)</span>
                                                    <?php endif; ?>
                                                </p>
                                                <?php endif; ?>
                                            </td>
