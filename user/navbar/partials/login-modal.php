        ...
        <div x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 scale-95"
            ...
            <div class="p-6">
                <form x-data="loginForm()" @submit.prevent="handleLogin($event)" class="space-y-4">
                    ...
                    <div x-show="isEmail && isTrustedDevice && !otpSent && !otpVerified" x-transition
                        ...
                        <p class="text-xs text-green-700">Trusted device — enter your password to continue.</p>
                    </div>

                    <!-- Password field -->
                    <div x-show="isMobile || (isEmail && otpVerified) || (isEmail && isTrustedDevice)" x-transition class="space-y-2">
                        <label class="block text-sm font-medium text-gray-600">Password</label>
                        <input type="password" name="password" x-model="password" autocomplete="off"
                            class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500">
                    </div>

                    <!-- Send OTP button (only if email and NOT trusted) -->
                    <div x-show="isEmail && !isTrustedDevice && !otpSent && !otpVerified" x-transition class="space-y-2">
                    ...
                    <div x-show="otpSent && !otpVerified" x-transition class="space-y-3">
                        ...
                        <div class="text-center mt-2">
                            ...
                            <template x-if="resendCooldown === 0">
                                <button @click="sendOTP" class="text-blue-500 hover:underline text-sm font-medium" type="button">Resend OTP</button>
                            </template>
                        </div>
                    </div>
                    ...
                    <button type="submit" :disabled="submitLoading"
                        ...
                        x-show="isMobile || (isEmail && otpVerified) || (isEmail && isTrustedDevice)">
