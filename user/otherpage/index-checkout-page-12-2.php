...
<div class="bg-gray-100 font-sans">
            ...
            <div class="mb-8">
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center flex-1">
                        <div
                            class="w-10 h-10 bg-green-500 text-white rounded-full flex items-center justify-center shrink-0">
                        ...
                        <div class="ml-3 hidden sm:block">
                            <div class="font-medium text-green-600 text-sm">Customer Info</div>
                            <div class="text-xs text-gray-500">Completed</div>
                        </div>
                    </div>

                    <div class="flex-1 h-px bg-green-500 mx-2"></div>
                    ...
                    <div class="flex items-center flex-1">
                        <div
                            ...
                            2</div>
                        <div class="ml-3 hidden sm:block">
                            <div class="font-medium text-orange-600 text-sm">Delivery Address</div>
            ...
            <form method="POST" id="addressForm" class="space-y-6">
                ...
                <?php if ($has_billing_addresses): ?>
                    <div class="mb-6">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            <button type="button" id="openAddressModal"
                                class="bg-orange-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-orange-700 inline-flex items-center gap-2">
                        ...
                        <?php if ($active_address): ?>
                            <div class="border-2 border-green-500 rounded-lg p-4 bg-green-50">
                                <input type="hidden" name="billing_address_id" id="selectedAddressId"
                                    value="<?= $active_address['id'] ?>"
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center gap-2 mb-2">
                                            ...
                                            <div class="font-bold text-lg text-gray-800">
                                                <?= htmlspecialchars($active_address['full_name']) ?>
                                            </div>
                                        </div>
                                        <div class="text-orange-600 font-medium">
                                            <?= htmlspecialchars($active_address['phone']) ?>
                                        </div>
                                        <div class="text-gray-600 mt-1">
                                            <?= htmlspecialchars($active_address['address'] . ', ' . $active_address['city'] . ', ' . $active_address['state'] . ', ' . $active_address['country']) ?>
                                        </div>
                                        <div class="text-sm text-gray-500 mt-1">ZIP:
                                            <?= htmlspecialchars($active_address['postal_code']) ?>
                                        </div>

                                        <?php if (!empty($active_address['latitude']) && !empty($active_address['longitude'])): ?>
                                            <div class="text-xs text-gray-400 mt-1">
                                                <i class="fa-solid fa-magnifying-glass-location"></i>
                                                <?= number_format($active_address['latitude'], 6) ?>,
                                                <?= number_format($active_address['longitude'], 6) ?>
                                            </div>
                                        <?php endif; ?>
                                    </div>
                    ...
                    <div id="addressModal" class="hidden fixed items-center justify-center p-4"
                        ...
                        <div class="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] flex flex-col">
                            ...
                            <div class="p-6 overflow-y-auto flex-1">
                                <div class="mb-4 flex justify-end">
                                    <a href="profile-update_billing_add-page-7-A.php"
                                        class="bg-green-600 text-white px-4 py-2 rounded-lg text-sm hover:bg-green-700 inline-flex items-center gap-2">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <div class="space-y-3">
                                    <?php foreach ($billing_addresses as $addr): ?>
                                        <label
                                            class="flex items-start p-4 border-2 rounded-lg cursor-pointer hover:bg-orange-50 hover:border-orange-300 address-option transition <?= $addr['is_active'] ? 'border-green-500 bg-green-50' : '' ?>">
                                            <input type="radio" name="modal_address_radio" value="<?= $addr['id'] ?>"
                                                class="mt-2 mr-4" <?= $addr['is_active'] ? 'checked' : '' ?>
                                            ...
                                            <div class="flex-1">
                                                <div class="flex items-center justify-between mb-2">
                                                    <div class="flex items-center gap-2">
                                                        <?php if ($addr['is_active']): ?>
                                                            <span
                                                                class="bg-green-600 text-white text-xs px-2 py-1 rounded-full">ACTIVE</span>
                                                        <?php endif; ?>
                                                        <div class="font-bold text-lg text-gray-800">
                                                ...
                                                <div class="text-orange-600 font-medium"><?= htmlspecialchars($addr['phone']) ?>
                                                </div>
                                                <div class="text-gray-600 mt-1">
                                                    <?= htmlspecialchars($addr['address'] . ', ' . $addr['city'] . ', ' . $addr['state'] . ', ' . $addr['country']) ?>
                                                </div>
                                                <div class="text-sm text-gray-500 mt-1">ZIP:
                                                ...
                                                <?php if (!empty($addr['latitude']) && !empty($addr['longitude'])): ?>
                                                    <div class="text-xs text-gray-400 mt-1">
                                                        <i class="fa-solid fa-magnifying-glass-location"></i>
                                                        <?= number_format($addr['latitude'], 6) ?>,
                                                        <?= number_format($addr['longitude'], 6) ?>
                                                    </div>
                                                <?php endif; ?>
                                            </div>
                                        </label>
                            ...
                            <!-- Modal Footer -->
                            <div class="bg-gray-50 p-4 flex justify-end gap-3 shrink-0">
                ...
                <?php else: ?>
                    <div class="border-2 border-dashed border-red-300 rounded-lg p-8 text-center bg-red-50">
                        <svg class="mx-auto w-16 h-16 text-red-500 mb-4" fill="none" stroke="currentColor"
                            viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        ...
                        <p class="text-red-700 mb-6">You need to set up at least one delivery address to continue with your
                            order.</p>
                        <a href="profile-update_billing_add-page-7-A.php"
                            class="inline-flex items-center gap-2 bg-red-600 text-white px-8 py-3 rounded-lg hover:bg-red-700 transition font-medium">
                ...
                <?php if ($has_billing_addresses): ?>
                    <div class="flex justify-between items-center pt-4">
                        <a href="<?= BASE_URL ?>/checkout" class="text-gray-600 hover:text-gray-800 flex items-center">
    ...
    <script
        src="<?= BASE_URL ?>/user/js/index-checkout-addressZone-page-12-2.obfuscated.js?v=<?= filemtime('js/index-checkout-addressZone-page-12-2.obfuscated.js') ?>"></script>
