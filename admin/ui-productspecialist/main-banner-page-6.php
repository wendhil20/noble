...
<body class="bg-gray-50 min-h-screen">
  ...
  <div class="max-w-7xl mx-auto p-6">
    ...
    <form action="" method="post" enctype="multipart/form-data"
      class="bg-white shadow rounded-lg p-6 mb-10 border border-blue-200">
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
        ...
        <div>
          ...
          <input type="file" name="images[]" multiple required
            class="w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:border-0 file:text-sm file:font-semibold file:bg-blue-50 file:text-blue-700 hover:file:bg-blue-100" />
        </div>
      </div>
      ...
      while ($row = $result->fetch_assoc()):
        ...
        <div
          ...
          <div class="mb-3 p-2 bg-gray-50 rounded text-[11px] space-y-1.5">
            ...
            <div>
              <span
                class="inline-block text-[9px] font-semibold <?= (isset($row['is_active']) && $row['is_active']) ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-700' ?> px-2 py-1 rounded">
          ...
          <form action="" method="post" enctype="multipart/form-data" class="mb-2">
            <div class="space-y-2">
              ...
              <input type="file" name="update_image"
                class="block w-full text-sm text-gray-600 file:mr-4 file:py-1 file:px-3 file:border-0 file:rounded file:bg-yellow-100 file:text-yellow-800 hover:file:bg-yellow-200" />
              <input type="hidden" name="update_id" value="<?= $row['id'] ?>" />
              <button type="submit"
                class="w-full bg-yellow-500 hover:bg-yellow-600 text-white text-xs font-medium px-3 py-1 rounded">
                Update
              </button>
            </div>
          </form>
          ...
          <form action="" method="post" class="mb-2">
            ...
            <button type="submit"
              class="w-full <?= (isset($row['is_active']) && $row['is_active']) ? 'bg-green-500 hover:bg-green-600' : 'bg-gray-500 hover:bg-gray-600' ?> text-white text-xs font-medium px-3 py-1 rounded">
              <?= (isset($row['is_active']) && $row['is_active']) ? '✓ Active' : '○ Inactive' ?>
            </button>
          </form>
          ...
          <form action="" method="post" onsubmit="return confirm('Delete this image?')">
            ...
            <button type="submit"
              class="w-full text-center bg-red-500 hover:bg-red-600 text-white text-xs font-medium px-3 py-1 rounded">
              Delete
            </button>
          </form>
        </div>
    ...
    <form action="" method="post" enctype="multipart/form-data"
      class="bg-white shadow rounded-lg p-6 mb-10 border border-orange-200">
      <label class="block text-sm font-medium text-gray-700 mb-2">Upload On Sale Banner Images</label>
      <input type="file" name="onsale_images[]" multiple required
        class="mb-4 w-full text-sm text-gray-700 file:mr-4 file:py-2 file:px-4 file:border-0 file:text-sm file:font-semibold file:bg-orange-50 file:text-orange-700 hover:file:bg-orange-100" />
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
      ...
      while ($row = $result_onsale->fetch_assoc()):
        ...
        <div class="bg-white rounded-lg shadow hover:shadow-md p-4 relative border border-orange-100">
          <img src="<?= BASE_URL ?>/<?= basename($row['filename']) ?>" alt="onsale banner"
            class="w-full h-40 object-contain mb-4 rounded-md border" />

          <!-- Update Form -->
          <form action="" method="post" enctype="multipart/form-data" class="flex flex-col space-y-2 mb-2">
            <input type="file" name="update_onsale_image" required
              class="block w-full text-sm text-gray-600 file:mr-4 file:py-1 file:px-3 file:border-0 file:rounded file:bg-yellow-100 file:text-yellow-800 hover:file:bg-yellow-200" />
            <input type="hidden" name="update_onsale_id" value="<?= $row['id'] ?>" />
            <button type="submit"
              class="bg-yellow-500 hover:bg-yellow-600 text-white text-xs font-medium px-3 py-1 rounded">
              Update
            </button>
          </form>
