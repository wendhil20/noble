...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    ...
    if ($_POST['action'] === 'start_transit') {
        ...
        try {
            ...
            $updateDS = $conn->prepare("UPDATE delivery_schedules SET delivery_status = 'out_for_delivery' WHERE id = ?");
            $updateDS->bind_param("i", $booking['delivery_schedule_id']);
            $updateDS->execute();
            $updateDS->close();

            $conn->commit();
            $_SESSION['success_message'] = "Delivery started! Status changed to In Transit.";
        } catch (Exception $e) {
            $conn->rollback();
...
<body class="bg-gradient-to-br from-gray-50 to-blue-50 min-h-screen">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        ...
        <div class="mb-4">
            ...
            <p class="text-gray-600 mt-1 text-sm">Order #<?php echo $booking['order_id']; ?> - Booking #<?php echo $booking_id; ?></p>
        </div>
            ...
            <!-- Main Content -->
                ...
                <?php if ($allLoaded && !$isInTransit && !$isCompleted): ?>
                    <div class="bg-gradient-to-r from-green-500 to-green-600 rounded-xl shadow-lg p-4 text-white">
                        ...
                        <p class="mb-3 text-green-100 text-sm">All items are loaded. Click below to mark as "In Transit"</p>

                        <form method="POST" onsubmit="return confirm('Are you sure all items are loaded and ready to go?');">
                ...
                <?php if ($isInTransit): ?>
                    <div class="bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl shadow-lg p-4 text-white">
                        ...
                        <p class="text-blue-100 text-sm">This delivery is currently in transit. Drive safely!</p>
                    </div>
                <?php endif; ?>
            </div>
        </div>
