        ...
        if (is_array($subImagesArray)) {
          foreach ($subImagesArray as $subImage) {
            if (!empty($subImage)) {
              ...
              $possiblePaths = ["../../" . $cleanPath, $subImage, "./" . $cleanPath];
              foreach ($possiblePaths as $filePath) {
                if (file_exists($filePath)) { $imagesToDelete[] = $filePath; break; }
              }
            }
          }
        }
...
// Collect into PHP arrays for JS injection
$subcategories_with_category   = [];
while ($sub  = $subcategory_result->fetch_assoc())   { $subcategories_with_category[]   = $sub;  }

$sub_subcategories_with_parent = [];
while ($subsub = $sub_subcategory_result->fetch_assoc()) { $sub_subcategories_with_parent[] = $subsub; }
...
// ── Build products query (simple, no correlated subqueries) ──────────────────
$productsQuery = "
  SELECT
    p.id,
    p.product_name,
...
$productsQuery .= "
  GROUP BY p.id, p.product_name, p.codename, p.quantity,
           p.main_image, p.created_at, p.updated_at, p.is_archived,
           cat.id, cat.name
...
// JSON sub_subcategory_ids array
...
<head>
  ...
  <style>
    ...
    .table-row-hover:hover { background-color: #faf8f6; }
    .sticky-header { position: sticky; top: 0; z-index: 10; }
    .product-image-cell { width: 60px; height: 60px; object-fit: contain; background: #f9fafb; border-radius: 0.5rem; }
    .archived-row { opacity: 0.6; background-color: #fef3f2; }
    select[multiple] { padding: 4px; min-height: 32px; }
    select[multiple] option:checked { background: linear-gradient(#f97316,#f97316); color: white; }
  </style>
</head>
...
<body class="bg-gradient-to-br from-gray-50 to-gray-100 min-h-screen font-roboto">
          ...
          <table class="w-full text-sm">
            ...
            <tbody id="productTableBody">
              <?php while ($product = $products->fetch_assoc()): ?>
                ...
                // Parse subcategory & sub-subcategory names for display
  ...
  <script>
    // ── Cascading filter data injected from PHP ──────────────────────────────
    ...
    function onCategoryChange(sel) {
      ...
      // reset sub-sub
      document.getElementById('filterSubSubcategory').innerHTML =
        '<option value="">All Sub-Subcategories</option>';
    ...
    // Populate product-code dropdown from rendered rows
    ...
    function filterRows() {
      ...
      rows.forEach(row => {
        ...
        const matchStock  = quantityValue === 'all'
          ? true
          : quantityValue === 'in-stock' ? stock > 0 : stock === 0;
      ...
      if (visible === 0 && !noResult) {
        ...
        tr.innerHTML = '<td colspan="11" class="px-6 py-8 text-center text-gray-500"><i class="fas fa-search mr-2"></i>No products found</td>';
