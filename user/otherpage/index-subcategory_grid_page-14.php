...
foreach ($subcategories as $sub) {
    if ($count >= 4)
        break; // Only get 4 subcategories
    ...
    <div class="bg-white border-b">
        <div class="container mx-auto px-6 py-4">
            <nav class="flex items-center text-sm text-gray-600">
                <a href="<?= BASE_URL ?>/" class="hover:text-gray-900">Home</a>
                ...
                <?php foreach ($subcategories as $sub): ?>
                    <div class="space-y-4">
                        ...
                        <div class="bg-white rounded-lg overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300">
                            ...
                            <!-- Image Container -->
                            <a href="<?= BASE_URL ?>/recommended?subcategory_id=<?= $sub['id'] ?>&from=grid"
                                class="block relative bg-gray-50 overflow-hidden group" style="height: 200px;">

                                <!-- Visual indicator -->
                                <div
                                    class="absolute top-2 left-2 bg-black text-white px-2 py-1 rounded-md text-xs  z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                                    Recommended
                                </div>

                                <?php if (!empty($sub['image_path'])): ?>
                                    <img src="<?= BASE_URL ?>/uploads/<?= htmlspecialchars($sub['subcategory_slug']) ?>/<?= htmlspecialchars($sub['image_path']) ?>"
                            ...
                            <div class="pl-2">
                                <ul class="space-y-1.5 text-sm">
                                    <?php foreach ($sub['collections'] as $collection): ?>
                                        <li>
                                            <a href="<?= BASE_URL ?>/productsubviews?sub_subcategory_id=<?= $collection['id'] ?>"
                    ...
                    <div class="mb-16">
                        ...
                        <div class="mb-6">
                            <a href="<?= BASE_URL ?>/productsubviews?sub_subcategory_id=<?= $collection['id'] ?>"
                                ...
                                <i
                                    class="fas fa-arrow-right ml-3 text-lg opacity-0 group-hover:opacity-100 transition-opacity"></i>
                            </a>
                        </div>
                        ...
                        <?php if (!empty($collection['products'])): ?>
                            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 gap-4 mb-6">
                                <?php foreach ($collection['products'] as $product): ?>
                                    ...
                                    <a href="<?= BASE_URL ?>/productview?id=<?= $product['id'] ?>"
