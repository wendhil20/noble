...
foreach ($orders as $order) {
    ...
    if ((int) $order['approved_replacements_count'] > 0)
        $ordersWithReplacements++;
    if ((int) $order['unresolved_defects_count'] > 0)
        $ordersWithDefects++;
}
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition-shadow">
                <div class="flex items-center justify-between">
                    <div>
                        ...
                        <p
                            class="text-3xl font-bold text-<?php echo ($ordersWithDefects + $ordersWithReplacements) > 0 ? 'red' : 'gray'; ?>-600">
                            <?php echo $ordersWithDefects + $ordersWithReplacements; ?></p>
                        <p class="text-xs text-gray-500 mt-1"><?php echo $ordersWithReplacements; ?> replacements,
                            <?php echo $ordersWithDefects; ?> defects</p>
                    </div>
            ...
            <form method="GET" class="space-y-4">
                ...
                <div>
                    <div class="flex flex-wrap gap-2 mb-4">
                        <a href="?"
                            class="inline-flex items-center px-4 py-2 rounded-lg text-sm font-medium transition-colors <?php echo ($status_filter === '' && !$show_active) ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'; ?>">
                            <i class="fas fa-th mr-2"></i>
                            All Orders
                            <span
                                class="ml-2 px-2 py-0.5 rounded-full text-xs font-bold <?php echo ($status_filter === '' && !$show_active) ? 'bg-white/20' : 'bg-gray-200'; ?>">
                        ...
                        <a href="?active=1<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?><?php echo !empty($date_from) ? '&date_from=' . urlencode($date_from) : ''; ?><?php echo !empty($date_to) ? '&date_to=' . urlencode($date_to) : ''; ?><?php echo $warehouse_filter > 0 ? '&warehouse=' . $warehouse_filter : ''; ?>"
                            ...
                            Active Orders
                            <span
                                class="ml-2 px-2 py-0.5 rounded-full text-xs font-bold <?php echo $show_active ? 'bg-white/20' : 'bg-green-200'; ?>">
                        ...
                        foreach ($statusOrder as $status):
                            ...
                            <a href="?status=<?php echo urlencode($status); ?><?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?><?php echo !empty($date_from) ? '&date_from=' . urlencode($date_from) : ''; ?><?php echo !empty($date_to) ? '&date_to=' . urlencode($date_to) : ''; ?><?php echo $warehouse_filter > 0 ? '&warehouse=' . $warehouse_filter : ''; ?>"
                                ...
                                <span
                                    class="ml-2 px-2 py-0.5 rounded-full text-xs font-bold <?php echo $isActive ? 'bg-white/20' : 'bg-' . $config['color'] . '-200'; ?>">
                                    <?php echo $count; ?>
                                </span>
                            </a>
                        <?php endforeach; ?>
                ...
                <div class="flex justify-end space-x-3 pt-2">
                    <a href="?"
                        class="bg-gray-500 hover:bg-gray-600 text-white px-6 py-2 rounded-lg transition-colors flex items-center space-x-2 text-sm">
                        <i class="fas fa-redo"></i>
                        <span>Reset</span>
                    </a>
                    <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white px-6 py-2 rounded-lg transition-colors flex items-center space-x-2 text-sm">
            ...
            <div class="bg-white rounded-lg border border-gray-200 p-5 mb-4">
                ...
                <div class="space-y-2">
                    <?php foreach ($orders as $order):
                        ...
                        ?>
                        <div
                            class="border <?php echo $hasIssues ? 'border-l-4 border-l-red-500 border-gray-200' : 'border-gray-200'; ?> bg-white rounded-lg p-3 hover:shadow-md transition-shadow">
                            <div class="flex items-center justify-between">
                                <!-- Left: Order Info -->
                                <div class="flex items-center space-x-4 flex-1">
                                    <div class="text-center min-w-[80px]">
                                        ...
                                        <div class="text-lg font-bold text-gray-900">
                                            #<?php echo htmlspecialchars($order['id']); ?></div>
                                    </div>
                                    ...
                                    <div class="flex-1">
                                        ...
                                        <div class="text-xs text-gray-500">
                                            <?php echo date('M j, Y g:i A', strtotime($order['created_at'])); ?></div>
                                    </div>
                                </div>
                                ...
                                <div class="flex items-center space-x-2 flex-wrap gap-y-1">
                                    ...
                                    <?php else: ?>
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded text-xs font-semibold <?php echo $statusColor; ?>">
                                    ...
                                    <?php if ($approved_replacements_count > 0): ?>
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded text-xs font-bold bg-red-600 text-white"
                                    ...
                                    if ($warehouse_replacements > 0): ?>
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded text-xs font-bold bg-amber-500 text-white"
                                    ...
                                    <?php if ($unresolved_defects_count > 0): ?>
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded text-xs font-bold bg-orange-600 text-white"
                                    ...
                                    <span
                                        class="text-xs font-semibold <?php echo ($assignmentPercentage === 100) ? 'text-green-600' : (($assignmentPercentage > 0) ? 'text-yellow-600' : 'text-red-600'); ?>">
                                ...
                                <div class="flex items-center space-x-4 ml-4">
                                    ...
                                    <a href="<?= BASE_URL ?>/ownerorders?order_id=<?php echo $order['id']; ?>"
