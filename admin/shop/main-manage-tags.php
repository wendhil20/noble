...
include '../../user/navbar/main-tag-helpers.php';
...
$usage = ['normal' => 0];
foreach ($all_tags as $slug => $_) $usage[$slug] = 0;
foreach (['categories', 'product_subcategories', 'product_sub_subcategories'] as $t) {
...
<head>
  ...
  <style>
    .tag-pill { display:inline-flex;align-items:center;gap:4px;font-size:11px;font-weight:700;padding:2px 10px;border-radius:99px;white-space:nowrap; }
    .color-swatch { width:22px;height:22px;border-radius:5px;border:1px solid #e5e7eb;display:inline-block;vertical-align:middle; }
...
<div class="max-w-5xl mx-auto p-6">
  <div class="bg-white rounded-xl shadow-lg p-8">
    ...
    <div class="flex items-center justify-between mb-6">
      ...
      <a href="main-category-product-page-3.php" class="bg-gray-600 hover:bg-gray-700 text-white px-4 py-2 rounded-lg text-sm font-medium transition">← Back</a>
    </div>

    <?php if ($message): ?>
      <div class="bg-green-50 border border-green-200 text-green-800 px-4 py-3 rounded-lg mb-5"><?= htmlspecialchars($message) ?></div>
    <?php endif; ?>
    <?php if ($error): ?>
      <div class="bg-red-50 border border-red-200 text-red-800 px-4 py-3 rounded-lg mb-5"><?= htmlspecialchars($error) ?></div>
    <?php endif; ?>
    ...
    <div class="bg-gray-50 border border-gray-200 rounded-xl p-6 mb-8">
      ...
      <form method="POST" class="grid grid-cols-2 md:grid-cols-3 gap-4">
        ...
        <div class="md:col-span-3 flex items-center gap-4 pt-2">
          <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg text-sm font-medium transition">+ Add Tag</button>
          <span class="text-xs text-gray-400">Preview:</span>
          <span id="add_preview" class="tag-pill" style="background:#ffae00;color:#ffffff;border:1px solid #ffae00;">
            <span id="add_dot_el" style="width:5px;height:5px;border-radius:50%;background:#ffffff;display:inline-block;"></span>
        ...
        <tbody class="divide-y divide-gray-100">
          ...
          <tr class="hover:bg-gray-50">
            ...
            <td class="px-4 py-3 text-center">
              <span class="bg-gray-100 text-gray-500 text-xs font-semibold px-2 py-0.5 rounded-full"><?= $usage['normal'] ?? 0 ?></span>
            </td>
          ...
          <tr class="hover:bg-gray-50" id="row-<?= htmlspecialchars($slug) ?>">
            ...
            <td class="px-4 py-3">
              <span class="tag-pill" style="background:<?= $bg ?>;color:<?= $color ?>;border:1px solid <?= $border ?>;">
            ...
            <td class="px-4 py-3 text-center">
              <span class="<?= $count > 0 ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-400' ?> text-xs font-semibold px-2 py-0.5 rounded-full">
                <?= $count ?>
              </span>
            </td>
            <td class="px-4 py-3 text-center">
              <div class="flex justify-center gap-2">
                ...
                <form method="POST" class="inline"
                  ...
                  <button type="submit" class="bg-red-500 hover:bg-red-600 text-white px-3 py-1 rounded text-xs font-medium transition">Delete</button>
                </form>
              </div>
            </td>
          </tr>
            ...
            <td colspan="6" class="px-4 py-4">
              <form method="POST" class="grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
                ...
                <div class="md:col-span-4 flex gap-3 pt-1">
                  <button type="submit" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-1.5 rounded-lg text-sm font-medium transition">Save</button>
    ...
    <div class="mt-6 bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm text-amber-800">
      <strong>Note:</strong> Tag columns are now <code class="bg-amber-100 px-1 rounded">VARCHAR(50)</code> — no <code class="bg-amber-100 px-1 rounded">ALTER TABLE</code> needed when adding/deleting tags. Just manage rows in <code class="bg-amber-100 px-1 rounded">tag_config</code>.
    </div>
  </div>
</div>
