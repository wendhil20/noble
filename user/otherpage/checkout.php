...
<body class="bg-gray-100 font-sans">
    ...
    `;
        ...
        // Modified auto-detect function para sa display-only output
        function autoDetectAndSelectZone(postalCode) {
            ...
            // Enhanced postal code zone detection (same logic mo)
            if (postal_num >= 1000 && postal_num <= 1800) {
                targetZoneCode = 'NCR';
            ...
            const deliveryZones = [
                ...
                foreach ($delivery_zones as $zone) {
                    $zone_items[] = '{
            id: "' . $zone['id'] . '",
            zone_name: "' . addslashes($zone['zone_name']) . '",
            zone_code: "' . addslashes($zone['zone_code']) . '",
            base_fee: ' . $zone['base_fee'] . ',
            included_km: ' . $zone['included_km'] . ',
            per_km_rate: ' . $zone['per_km_rate'] . ',
            is_free_delivery: ' . ($zone['is_free_delivery'] ? 'true' : 'false') . '
        }';
                }
                echo implode(',', $zone_items);
                ?>
            ];

            // Find matching zone
            const matchedZone = deliveryZones.find(zone => zone.zone_code === targetZoneCode);
            ...
            if (matchedZone) {
                ...
                // Show notification (if may function ka pa nito)
    ...
    `;
        ...
        // Function para sa manual override if needed (optional)
        function setDeliveryZoneManual(zoneId) {
            const deliveryZones = [
                ...
                foreach ($delivery_zones as $zone) {
                    $zone_items[] = '{
                id: \'' . $zone['id'] . '\',
                zone_name: \'' . htmlspecialchars($zone['zone_name']) . '\',
                zone_code: \'' . htmlspecialchars($zone['zone_code']) . '\',
                base_fee: ' . $zone['base_fee'] . ',
                included_km: ' . $zone['included_km'] . ',
                per_km_rate: ' . $zone['per_km_rate'] . ',
                is_free_delivery: ' . ($zone['is_free_delivery'] ? 'true' : 'false') . '
