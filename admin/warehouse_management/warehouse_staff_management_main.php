...
<body class="bg-gray-100 min-h-screen">
                    ...
                    <?php if ($is_head): ?>
                        <div class="mt-2">
                            <a href="warehouse_head_assignment.php"
                                class="bg-gradient-to-r from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white px-4 py-2 rounded-lg transition-all duration-200 flex items-center justify-center space-x-2 shadow-lg">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-6">
            <form method="GET" class="space-y-4">
                ...
                <div class="flex flex-wrap gap-2 mb-4">
                    ...
                    <a href="?"
                        class="px-3 py-1.5 rounded-lg text-xs font-medium transition-all duration-200 inline-flex items-center gap-1.5 <?php echo ($status_filter === '' && !$show_replacements && !$show_ready_for_schedule && !$show_defects && !$show_ready_replacements) ? 'bg-gray-100 shadow-sm' : 'bg-orange-100 text-orange-700 hover:bg-orange-200'; ?>">
                        <i class="fas fa-tasks text-sm"></i>
                        <span>Incomplete</span>
                        <span
                            class="ml-1 px-1.5 py-0.5 rounded text-xs <?php echo ($status_filter === '' && !$show_replacements && !$show_ready_for_schedule && !$show_defects && !$show_ready_replacements) ? 'bg-white/20' : 'bg-orange-200'; ?>">
                    ...
                    foreach ($statusOrder as $status):
                        ...
                        <a href="?status=<?php echo urlencode($status); ?><?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?><?php echo !empty($date_from) ? '&date_from=' . urlencode($date_from) : ''; ?><?php echo !empty($date_to) ? '&date_to=' . urlencode($date_to) : ''; ?>"
                            ...
                            <span
                                class="ml-1 px-1.5 py-0.5 rounded text-xs <?php echo $isActive ? 'bg-white/20' : 'bg-' . $config['color'] . '-200'; ?>">
                                <?php echo (int) $count; ?>
                            </span>
                        </a>
                    <?php endforeach; ?>
                    ...
                    <?php if ($replacementOrdersCount > 0): ?>
                        <a href="?replacements=1<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?><?php echo !empty($date_from) ? '&date_from=' . urlencode($date_from) : ''; ?><?php echo !empty($date_to) ? '&date_to=' . urlencode($date_to) : ''; ?>"
                            ...
                            <span
                                class="ml-1 px-1.5 py-0.5 rounded text-xs <?php echo $show_replacements ? 'bg-white/20' : 'bg-red-200'; ?>">
                                <?php echo $replacementOrdersCount; ?>
                            </span>
                            <?php if (!$show_replacements): ?>
                                <span
                                    class="absolute -top-1 -right-1 h-2 w-2 bg-red-500 border border-white rounded-full animate-pulse"></span>
                            <?php endif; ?>
                        </a>
                    <?php endif; ?>
            ...
            ";
                    ...
                    if ($defectsOrdersCount > 0): ?>
                        <a href="?defects=1<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?><?php echo !empty($date_from) ? '&date_from=' . urlencode($date_from) : ''; ?><?php echo !empty($date_to) ? '&date_to=' . urlencode($date_to) : ''; ?>"
                            ...
                            <span>Defects</span>
                            <span
                                class="ml-1 px-1.5 py-0.5 rounded text-xs <?php echo $show_defects ? 'bg-white/20' : 'bg-orange-200'; ?>">
                                <?php echo $defectsOrdersCount; ?>
                            </span>
                            <?php if (!$show_defects): ?>
                                <span
                                    class="absolute -top-1 -right-1 h-2 w-2 bg-orange-500 border border-white rounded-full animate-pulse"></span>
                            <?php endif; ?>
                        </a>
                    <?php endif; ?>
                    ...
                    <?php if ($readyForScheduleCount > 0): ?>
                        <a href="?ready_schedule=1<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?><?php echo !empty($date_from) ? '&date_from=' . urlencode($date_from) : ''; ?><?php echo !empty($date_to) ? '&date_to=' . urlencode($date_to) : ''; ?>"
                            ...
                            <span>Schedule</span>
                            <span
                                class="ml-1 px-1.5 py-0.5 rounded text-xs <?php echo $show_ready_for_schedule ? 'bg-white/20' : 'bg-green-200'; ?>">
                                <?php echo $readyForScheduleCount; ?>
                            </span>
                            <?php if (!$show_ready_for_schedule): ?>
                                <span
                                    class="absolute -top-1 -right-1 h-2 w-2 bg-green-500 border border-white rounded-full animate-pulse"></span>
                            <?php endif; ?>
                        </a>
                    <?php endif; ?>
            ...
            ";
                    ...
                    if ($readyToOrderFilterCount > 0): ?>
                        <a href="?ready_to_order=1<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?><?php echo !empty($date_from) ? '&date_from=' . urlencode($date_from) : ''; ?><?php echo !empty($date_to) ? '&date_to=' . urlencode($date_to) : ''; ?>"
                            ...
                            <span
                                class="ml-1 px-1.5 py-0.5 rounded text-xs <?php echo $show_ready_to_order ? 'bg-white/20' : 'bg-green-200'; ?>">
                    ...
                    <?php if ($readyReplacementsCount > 0): ?>
                        <a href="?ready_replacements=1<?php echo !empty($search_query) ? '&search=' . urlencode($search_query) : ''; ?><?php echo !empty($date_from) ? '&date_from=' . urlencode($date_from) : ''; ?><?php echo !empty($date_to) ? '&date_to=' . urlencode($date_to) : ''; ?>"
                            ...
                            <span>Rep. Ready</span>
                            <span
                                class="ml-1 px-1.5 py-0.5 rounded text-xs <?php echo $show_ready_replacements ? 'bg-white/20' : 'bg-red-200'; ?>">
                ...
                <div class="flex gap-2">
                    ...
                    <button type="submit"
                        class="px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-sm font-medium transition-all duration-200 flex items-center gap-2">
        ...
        <?php if (!empty($orders)): ?>
            <div class="space-y-4">
                <?php foreach ($orders as $order):
                    ...
                    ?>
                    <div
                        class="bg-white rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition-shadow duration-200 <?php echo $hasReplacements ? 'border-l-4 border-l-red-500' : ''; ?>">
                        <div class="p-6">
                            <div class="flex items-center justify-between">
                                <div class="flex items-start space-x-4">
                                    <div class="bg-gray-600 p-2 rounded-lg relative">
                                        <i class="fa-solid fa-dolly text-white"></i>
                                        <?php if ($hasReplacements): ?>
                                            <span
                                                class="absolute -top-2 -right-2 h-5 w-5 bg-red-500 border-2 border-white rounded-full flex items-center justify-center pulse-notification shadow-lg">
                                    ...
                                    <div>
                                        <div class="flex items-center flex-wrap gap-2 mb-1">
                                            ...
                                            <?php if ($hasReplacements): ?>
                                                <span
                                                    class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 pulse-notification">
                                                    <i class="fas fa-sync-alt mr-1"></i>
                                                    <?php echo $approved_replacements_count; ?>
                                                    Replacement<?php echo $approved_replacements_count > 1 ? 's' : ''; ?>
                                                </span>
                                            <?php endif; ?>
                                            <?php if ($unresolved_defects_count > 0): ?>
                                                <span
                                                    class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800 pulse-notification">
                                                    <i class="fas fa-exclamation-triangle mr-1"></i>
                                                    <?php echo $unresolved_defects_count; ?>
                                                    Defect<?php echo $unresolved_defects_count > 1 ? 's' : ''; ?>
                                                </span>
                                            <?php endif; ?>
                                            <?php
                ...
                ";
                                            ...
                                            if ($isReadyForSchedule): ?>
                                                <span
                                                    class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 pulse-notification relative">
    ...
    ";
                                            ...
                                            if ($replacementsReady): ?>
                                                <span
                                                    class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-red-100 text-red-800 pulse-notification relative">
                                        ...
                                        <div class="text-sm text-gray-600 space-y-1">
                                            <div class="flex items-center"><i
                                                    class="fas fa-user mr-2"></i><span><?php echo htmlspecialchars($order['customer_name']); ?></span>
                                            </div>
                                            <div class="flex items-center"><i
                                                    class="fas fa-envelope mr-2"></i><span><?php echo htmlspecialchars($order['email']); ?></span>
                                            </div>
                                            <div class="flex items-center"><i
                                                    class="fas fa-calendar mr-2"></i><span><?php echo date('M j, Y g:i A', strtotime($order['created_at'])); ?></span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="text-right space-y-2">
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <span
                                            class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium <?php echo ($order['status'] === 'pending') ? 'bg-yellow-100 text-yellow-800' : (($order['status'] === 'processing') ? 'bg-blue-100 text-blue-800' : (($order['status'] === 'completed') ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-800')); ?>"><?php echo htmlspecialchars(ucfirst($order['status'])); ?></span>
                                    ...
                                    <div class="text-xs">
                                        <div class="flex items-center justify-between mb-1">
                                            ...
                                            <span
                                                class="font-medium <?php echo ($assignmentPercentage === 100) ? 'text-green-600' : (($assignmentPercentage > 0) ? 'text-yellow-600' : 'text-red-600'); ?>"><?php echo $assignmentPercentage; ?>%</span>
                                        </div>
                                        ...
                                        <div class="mt-1 text-gray-500"><?php echo $assignedItems; ?>/<?php echo $item_count; ?>
                                            assigned</div>
                        ...
                        AND marked_as_ordered = 0";
                                            ...
                                            if ($readyToOrderCount > 0): ?>
                                                <div class="mt-1 flex items-center">
                                                    <span
                                                        class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800 pulse-notification">
                                ...
                                <div class="flex flex-col items-center space-y-2 ml-4">
                                   <?php if ($hasReplacements): ?>
    <a href="warehouse_staff_order_tracking_C.php?order_id=<?php echo urlencode($order['id']); ?>"
        class="bg-red-600 hover:bg-red-700 text-white px-3 py-2 rounded-lg transition-colors duration-200 flex items-center space-x-2 text-sm pulse-notification relative">
        <i class="fas fa-sync-alt"></i>
        <span>Manage Replacements (<?php echo $approved_replacements_count; ?>)</span>
...
<?php endif; ?>
    ...
    FROM po_attachments WHERE order_id = ?";
                                    ...
                                    <a href="warehouse_staff_po_management_A.php?order_id=<?php echo urlencode($order['id']); ?>"
                                        class="bg-blue-200 hover:bg-blue-400 text-black px-4 py-2 rounded-lg transition-colors duration-200 flex items-center space-x-2">
    ...
    // For completed orders, show Track Items regardless of marked_as_ordered
    ...
    if ($allPOsOrderedCheck):
                                            ?>
                                            <a href="warehouse_staff_order_tracking_C.php?order_id=<?php echo urlencode($order['id']); ?>"
                                                class="bg-purple-600 hover:bg-purple-700 text-white px-3 py-2 rounded-lg transition-colors duration-200 flex items-center space-x-2 text-sm">
                                    ...
                                    <?php if ($unresolved_defects_count > 0): ?>
                                        <a href="view_defects.php?order_id=<?php echo urlencode($order['id']); ?>"
                                            ...
                                            <span
                                                class="absolute -top-1 -right-1 h-3 w-3 bg-orange-500 border-2 border-white rounded-full animate-ping"></span>
                                        </a>
                                    <?php endif; ?>
                                </div>
                            </div>
